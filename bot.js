...
// Возвращаем функцию вместо прямого создания экземпляра
module.exports = function initBot(customBot) {
    // Используем либо переданный экземпляр бота, либо создаем новый
    const token = process.env.TELEGRAM_BOT_TOKEN || '7417777601:AAGy92M0pjoizRHILNT7d72xMNf8a0BkKK8';
    
    // Используем переданный экземпляр, если есть, иначе создаем новый
    const bot = customBot || new TelegramBot(token, { polling: true });
...
🍂 Подтверждение отсутствия в других хаусах: ${form.other_houses}`;
    ...
    // В конце файла возвращаем экземпляр бота
    return bot;
}; 