...
// Функция для обработки обновлений из webhook
async function handleUpdate(update) {
    try {
        // Внутренний метод TelegramBot для обработки обновлений
        if (bot.processUpdate) {
            bot.processUpdate(update);
        } else {
            console.error('Метод processUpdate недоступен');
        }
    } catch (error) {
...
// Экспортируем бота и функцию обработки обновлений
module.exports = {
    bot,
    handleUpdate
};
